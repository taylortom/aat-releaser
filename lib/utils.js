      ...
      Utils.fetchGitHubData('milestones').then(data => {
        ...
        Utils.fetchGitHubData(`issues?milestone=${milestone.number}`).then(data => {
          ...
          data.reverse().forEach(i => {
            ...
            if(i.labels.find(l => l.id === 680840117)) inProgress.push(i);
            else if(i.labels.find(l => l.id === 680840253)) awaitingReview.push(i);
            else if(!i.labels.find(l => l.id === 680840429)) unstarted.push(i);
