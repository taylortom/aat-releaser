...
module.exports = data => {
  ...
  return new Promise((resolve, reject) => {
    ...
    Utils.readFile(FILEPATH).then(contents => {
      const hMatch = contents.match(headerRE);
      const aMatch = contents.match(anchorRE);

      if(!hMatch || !aMatch) return Utils.exit(`Unexpected CHANGELOG file ${FILEPATH}.`);
